...
const humanizeCommentDate = (date) => humanizeDate(date, DATE_FORMATS.commentDate);

const createFilmGenresTemplate = (genres) => `${genres.map((genre) => `<span class="film-details__genre">${genre}</span>`).join('')}`;

const createCommentTemplate = (comments) => `${comments.map(({author, comment, emotion, date}) =>
...
const createNewCommentTemplate = () => `${commentsEmojis.map((emoji) =>
...
const createFilmPopupTemplate = (film) =>  {
  ...
  const filmRuntime = humanizeRunTime(filmInfo.runtime);
      ...
      <div class="film-details__info-wrap">
        ...
        <div class="film-details__info">
          ...
          <p class="film-details__film-description">
            ${filmInfo.description.length > 139 ? `${filmInfo.description.slice(0, 139)}...` : filmInfo.description}
          </p>
        </div>
      </div>
